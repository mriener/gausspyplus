...
for (y, x), fwhms, amps, means in zip(
    data["location"], decomp["fwhms_fit"], decomp["amplitudes_fit"], decomp["means_fit"]
):
    if fwhms is None:
        continue
...
for array, param, name in zip(
    ...
    filename = os.path.join(
        "decomposition_grs",
        "gpy_maps",
        "grs-test_field_g+_fit_fin_sf-p2-map_{}_fc.fits".format(name),
    )
    comments = [
        "GaussPy+ decomposition results.",
        "{} of first fit component.".format(param),
    ]
    header = update_header(header_pp, comments=comments, remove_old_comments=True)
    fits.writeto(filename, array.astype("float32"), header, overwrite=True)
