...
def test_decompose_cube_gausspy():
    ...
    decompose.decompose()
    with open(ROOT / 'tests' / 'test_grs/gpy_decomposed/grs-test_field_5x5_g_fit_fin.pickle', 'rb') as pfile:
        data_decomposed = pickle.load(pfile)
    assert np.array(data_decomposed['N_components']).sum() == 40
    assert np.allclose(sum(sum(lst) for lst in data_decomposed['fwhms_fit']), 698.5091534299819)
    assert np.allclose(sum(sum(lst) for lst in data_decomposed['fwhms_fit_err']), 86.91303941904955)

    decompose.improve_fitting = True
    decompose.suffix = '_g+'
    decompose.decompose()
    with open(ROOT / 'tests' / 'test_grs/gpy_decomposed/grs-test_field_5x5_g+_fit_fin.pickle', 'rb') as pfile:
        data_decomposed_gplus = pickle.load(pfile)
    assert np.array(data_decomposed_gplus['N_components']).sum() == 63
    assert np.allclose(sum(sum(lst) for lst in data_decomposed_gplus['fwhms_fit']), 1078.5970429518438)
    assert np.allclose(sum(sum(lst) for lst in data_decomposed_gplus['fwhms_fit_err']), 147.2004731702768)
...
def test_spatial_fitting_phase_1():
    ...
    with open(ROOT / 'tests' / 'test_grs/gpy_decomposed/grs-test_field_5x5_g+_fit_fin_sf-p1.pickle', 'rb') as pfile:
    ...
    assert np.allclose(sum(sum(lst) for lst in data_spatial_fitted_phase_1['fwhms_fit']), 1078.5459588039953)
    assert np.allclose(sum(sum(lst) for lst in data_spatial_fitted_phase_1['fwhms_fit_err']), 135.0725999894466)
    assert sum(data_spatial_fitted_phase_1['refit_iteration']) == 9
...
def test_spatial_fitting_phase_2():
    ...
    with open(ROOT / 'tests' / 'test_grs/gpy_decomposed/grs-test_field_5x5_g+_fit_fin_sf-p2.pickle', 'rb') as pfile:
        data_spatial_fitted_phase_2 = pickle.load(pfile)

    assert np.allclose(sum(sum(lst) for lst in data_spatial_fitted_phase_2['fwhms_fit']), 1078.5459588039953)
    assert np.allclose(sum(sum(lst) for lst in data_spatial_fitted_phase_2['fwhms_fit_err']), 135.0725999894466)
